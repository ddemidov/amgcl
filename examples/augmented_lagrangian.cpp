...
#include <amgcl/preconditioner/augmented_lagrangian.hpp>
...
void solve_al(const Matrix &K, std::vector<double> &rhs, boost::property_tree::ptree &prm)
{
    ...
    auto t1 = prof.scoped_tic("augmented_lagrangian");

    prof.tic("setup");
    amgcl::make_solver<
        amgcl::preconditioner::augmented_lagrangian<USolver, PSolver>,
...
#define AMGCL_BLOCK_PSOLVER(z, data, B)                                        \
  case B: {                                                                    \
    ...
    typedef amgcl::make_block_solver<                                          \
        ...
        PSolver;                                                               \
    solve_al<USolver, PSolver>(K, rhs, prm);                                   \
  } break;
...
int main(int argc, char *argv[]) {
    ...
    desc.add_options()
        ...
        (
         "pmass,W",
         po::value<string>()->required(),
         "The pressure mass matrix in MatrixMarket format"
        )
        (
         "pmask,m",
         po::value<string>(),
...

    {
        ...
        string Wfile  = vm["pmass"].as<string>();
        ...
        if (binary) {
            io::read_dense(Wfile, n, m, W);
        } else {
            std::tie(n, m) = io::mm_reader(Wfile)(W);
        }

        prm.put("precond.W", W.data());
    ...
    solve_al(vm["ub"].as<int>(), vm["pb"].as<int>(),
