...
#include <amgcl/preconditioner/uzawa.hpp>
...
void solve_uzawa(const Matrix &K, const std::vector<double> &rhs, boost::property_tree::ptree &prm)
{
    Backend::params bprm;
    ...
    auto t1 = prof.scoped_tic("uzawa");

    prof.tic("setup");
    amgcl::make_solver<
        amgcl::preconditioner::uzawa<USolver, PSolver>,
...
#define AMGCL_BLOCK_PSOLVER(z, data, B)                                        \
  case B: {                                                                    \
    ...
    typedef amgcl::make_block_solver<                                          \
        ...
        PSolver;                                                               \
    solve_uzawa<USolver, PSolver>(K, rhs, prm);                                \
  } break;

//---------------------------------------------------------------------------
template <class USolver, class Matrix>
void solve_uzawa(int pb, const Matrix &K, const std::vector<double> &rhs, boost::property_tree::ptree &prm)
{
    switch (pb) {
        case 1:
            {
...
#define AMGCL_BLOCK_USOLVER(z, data, B)                                        \
  case B: {                                                                    \
    ...
    typedef amgcl::make_block_solver<                                          \
        ...
        amgcl::runtime::solver::wrapper<BBackend> >                            \
        USolver;                                                               \
    solve_uzawa<USolver>(pb, K, rhs, prm);                                     \
  } break;

//---------------------------------------------------------------------------
template <class Matrix>
void solve_uzawa(int ub, int pb, const Matrix &K, const std::vector<double> &rhs, boost::property_tree::ptree &prm)
{
    switch (ub) {
        case 1:
            {
...
int main(int argc, char *argv[]) {
    ...
    solve_uzawa(vm["ub"].as<int>(), vm["pb"].as<int>(),
